...
class UrlManager extends BaseUrlManager
{
    ...
    public $column_language_id = 'language_id';
    public $column_language_code = 'language';
    public $column_language_status = 'status';
    ...
    public function getLanguageList(){
        $rows = (new \yii\db\Query())
            ->select([$this->column_language_id, $this->column_language_code])
            ->from("{{%$this->languageTable%}}")
            ->where([$this->column_language_status => 1])
            ->all();
        return ArrayHelper::map($rows,  $this->column_language_code, $this->column_language_id);
    ...
    public function getLangCode(){
        return $this->matchCode(Yii::$app->language)[0];
    }
    /**
    ...
    {
        $url_arr = explode('/',trim(\Yii::$app->request->url,'/'));
        foreach(\Yii::$app->urlManager->languages as $key=>$lang)
        {
            if(isset($url_arr[0]) && $key == $url_arr[0]){
                return $key;
            } elseif($lang == \Yii::$app->language && (!isset($url_arr[0]) || !array_key_exists($url_arr[0], \Yii::$app->urlManager->languages ))) {
                return $key;
            }
